...
Convert every letter to its ASCII value. Add 1 to the first letter, and then for every letter from the 
second one to the last one, add the value of the previous letter. Subtract 26 from every letter until 
it is in the range of lowercase letters a-z in ASCII. Convert the values back to letters.
...
function decrypt(word) {
    let asciiVal = word.charCodeAt(0)
    let res = '';
    let minAscii = 'a'.charCodeAt(0)
    if (word[0] !== 'a') {
      res += String.fromCharCode(asciiVal - 1)
    } else {
      res += 'z'
    }
    let prev = asciiVal
    for (let i = 1; i < word.length; i++) {
      let next = word.charCodeAt(i)
      console.log('1 : ', next)
      let nextRes = next - prev 
      while(nextRes < minAscii) {
    
        nextRes += 26
      }
      console.log('2 : ', nextRes)
      res += String.fromCharCode(nextRes)
      prev += nextRes
